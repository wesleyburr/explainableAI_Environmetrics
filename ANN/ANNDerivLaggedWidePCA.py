...
SSTPCATrain = SSTanom2[ :, 0:Lag1 ]

pca_top1 = PCA( n_components = 2 )
pca_top1.fit( SSTPCATrain )
pca_trs1 = pca_top1.fit_transform(SSTPCATrain)
...
# define base model
Soil1 = np.zeros(328)
SST1 = np.zeros( 2*3186)
SST1a = np.zeros( 6*3186)
for i in range(500):
    SoilTraintmp = SoilMoist1[:,(i+5)]
    Soil1 = np.vstack( (Soil1, SoilTraintmp) )
    SSTtmp1 = np.hstack( (np.reshape( SSTanom2[:,i], (3186,1)),
                              np.reshape(SSTanom2[:,(i+1)], (3186,1)),
                              np.reshape(SSTanom2[:,(i+2)], (3186,1)),
                              np.reshape(SSTanom2[:,(i+3)], (3186,1)),
                              np.reshape(SSTanom2[:,(i+4)], (3186,1)),
                              np.reshape(SSTanom2[:,(i + 5)], (3186,1)) )  )
    SSTtmp2 = pca_top1.fit_transform(SSTtmp1)
    SSTtmp3 = np.concatenate( ( SSTtmp2[:,0], SSTtmp2[:,1]))
    SST1 = np.vstack( (SST1, SSTtmp3))
    SSTtmp1a = np.concatenate(( SSTanom2[:,i],
                              SSTanom2[:,(i+1)],
                              SSTanom2[:,(i+2)],
                              SSTanom2[:,(i+3)],
                              SSTanom2[:,(i+4)],
                              SSTanom2[:,(i + 5)]))
    SST1a = np.vstack( (SST1a, SSTtmp1a))
    
Soil2 = np.delete( Soil1,0,axis = 0 )
SST2v = np.delete( SST1, 0, axis = 0 )
SST2va = np.delete( SST1a, 0, axis = 0 )


pca1 = PCA( n_components = 100 )
pca1.fit( SST2va )
SST2vT = pca1.transform( SST2va )
junk1trs0 = pca1.transform( np.reshape( SST2va[0,:], (1,6*3186) ) )
junk1trs0i = pca1.inverse_transform( junk1trs0 )
plt.grid()
...
SSTTrain = np.copy( SST2vT )
SSTFirst = SST2va[0,:]
SSTTrainSd1 = np.std(SST2va, axis = 0 )
...
for i in range( 0, Zn1 ):
   ...
   Ztesta = pca1.transform( np.reshape(Ztest, (1, 19116) ) )
   Zp = model.predict( Ztesta ) 
...
plt.scatter( df2.X, df2.Y, s = 2, c = df2.Z0, cmap = 'bwr') #, alpha = 0.5)
plt.colorbar()
plt.xlabel('lon')
plt.ylabel('lat')
plt.title('Lag 0 \n Using L=0 to 5')
#os.chdir("/Volumes/GoogleDrive/My Drive/Research/Working Group /SoilMoistureExample")
plt.savefig('/Volumes/GoogleDrive/My Drive/Research/Working Group /SoilMoistureExample/Model_ANN/Pred0PCA_L0to5out.png', format = 'png')#, quality = 100)
plt.show()

fig = plt.figure()
#plt.scatter( Xlatlon1[:,0], Xlatlon1[:,1], c = 'black', s = 1)
#plt.scatter( df2.X, df2.Y, c = 'white', s = 1 )
plt.scatter( df2.X, df2.Y, s = 2, c = df2.Z1, cmap = 'bwr') #, alpha = 0.5)
plt.colorbar()
plt.xlabel('lon')
plt.ylabel('lat')
plt.title('Lag 1 \n Using L=0 to 5')
...
#plt.scatter( Xlatlon1[:,0], Xlatlon1[:,1], c = 'black', s = 1)
#plt.scatter( df2.X, df2.Y, c = 'white', s = 1 )
plt.scatter( df2.X, df2.Y, s = 2, c = df2.Z2, cmap = 'bwr') #, alpha = 0.5)
plt.colorbar()
plt.xlabel('lon')
plt.ylabel('lat')
plt.title('Lag 2 \n Using L=0 to 5')
...
#plt.scatter( Xlatlon1[:,0], Xlatlon1[:,1], c = 'black', s = 1)
#plt.scatter( df2.X, df2.Y, c = 'white', s = 1 )
plt.scatter( df2.X, df2.Y, s = 2, c = df2.Z3, cmap = 'bwr') #, alpha = 0.5)
plt.colorbar()
plt.xlabel('lon')
plt.ylabel('lat')
plt.title('Lag 3 \n Using L=0 to 5')
...
#plt.scatter( Xlatlon1[:,0], Xlatlon1[:,1], c = 'black', s = 1)
#plt.scatter( df2.X, df2.Y, c = 'white', s = 1 )
plt.scatter( df2.X, df2.Y, s = 2, c = df2.Z4, cmap = 'bwr') #, alpha = 0.5)
plt.colorbar()
plt.xlabel('lon')
plt.ylabel('lat')
plt.title('Lag 4 \n Using L=0 to 5')
...
#plt.scatter( Xlatlon1[:,0], Xlatlon1[:,1], c = 'black', s = 1)
#plt.scatter( df2.X, df2.Y, c = 'white', s = 1 )
plt.scatter( df2.X, df2.Y, s = 2, c = df2.Z5, cmap = 'bwr') #, alpha = 0.5)fig = plt.figure()
plt.colorbar()
plt.xlabel('lon')
plt.ylabel('lat')
plt.title('Lag 5 \n Using L=0 to 5')
