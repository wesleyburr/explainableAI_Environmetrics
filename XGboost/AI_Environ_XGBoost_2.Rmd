...
```{r,results='hide'}
xgb.impt=matrix(nrow=nrow(corn.soil),ncol=315)
...
temp=xgb.importance(colnames(dtrain), model = fit_xgb)
temp2=temp %>% mutate(feat.var=str_sub(Feature,2,-1))
temp2=temp2[order(as.numeric(temp2$feat.var),decreasing = F),]
temp2$feat.var=as.numeric(temp2$feat.var)+1
xgb.impt[k,temp2$feat.var]=temp2$Gain
...
##Info for XGBoost Gain feature importance

overall.gain=apply(xgb.impt,2,mean,na.rm=T)
dat=cbind(seq(1,75),overall.gain[1:75])
colnames(dat)=c("Clust","Gain")
dat=data.frame(dat)
SST.clus2=data.frame(SST.clus)
colnames(SST.clus2)[1]="Clust"
SSTvis=left_join(SST.clus2[,1:3],dat,by="Clust")
...
ggplot(SSTvis,aes(x=Lon,y=Lat)) +               geom_point(data=SSTvis,aes(x=Lon,y=Lat,col=Gain),size=1.2) +
 scale_color_gradient2(low = "white", high = "red" ) + theme_classic()+
geom_hline(yintercept=c(0)) +
  labs(title="XGBoost Gain for Corn region")
