...
# define base model
SoilTrain =  np.transpose( SoilMoist1[:,Lag1:(789+Lag1+12)] )
SSTTrain = np.transpose( SSTanom2[:,0:(789+12)] )
SSTFirst = SSTTrain[0,:]
...
SSTtest = np.reshape( SSTanom2[:,(794+12)], (1, 3186) ) 
SoilTest = SoilMoist1[:,(797+12)]
...
# Write the data out...
df3.to_csv("Plots/ANNDerivRAWFeb_to_May_Ratio2015.csv", index = False)
...
plt.xlabel('Lon')
plt.ylabel('Lat')
plt.title('2015 May (RAW) $R^2_{pred}=$'+str(np.round(predR2,3)))
#os.chdir("/Volumes/GoogleDrive/My Drive/Research/Working Group /SoilMoistureExample")
plt.savefig('Plots/Pred_Feb_to_May_Ratio2015.png', format = 'png')#, quality = 100)
plt.show()
