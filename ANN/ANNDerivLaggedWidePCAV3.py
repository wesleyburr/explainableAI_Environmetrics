...
#################################################################################
# Build a model - Just add layers with the following code...

#. model.add( Dense( 420, activation = 'tanh' ) )
model = Sequential()
model.add( Dense( 360, input_dim = 60*7, activation = 'tanh' ) )
...
Z1 = SoilMoist1[:,796]
SSTFirst = np.concatenate( ( pca1.transform(SSTanom2[:,787].reshape([1,3186])).T,
                             pca1.transform(SSTanom2[:,788].reshape([1,3186])).T,
...
y1 = model.predict( np.reshape( SSTFirst, (1, 60*7) ) )
y1a = model.predict( np.reshape( SSTFirst, (1, 60*7) ) )*SoilMoist1s[(796)] + SoilMoist1bm[(796)] 
#MSE1 = MeanSquaredError()
#y1MSE = MSE1( Z1, y1 ).numpy()    #PredMSE
plt.figure()
plt.plot( y1.T, Z1, 'o' )
predR2 = sc.pearsonr(y1.T[:,0], Z1)[0]**2    # Pred R2
...
#os.chdir("/Volumes/GoogleDrive/My Drive/Research/Working Group /SoilMoistureExample")
plt.savefig('Plots/Pred800from799PCA_ratio.png', format = 'png')#, quality = 100)
plt.show()
...
plt.savefig('Plots/Pred800from798PCA_ratio.png', format = 'png')#, quality = 100)
plt.show()
...
plt.savefig('Plots/Pred800from797PCA_ratio.png', format = 'png')#, quality = 100)
plt.show()
...
plt.savefig('Plots/Pred800from796PCA_ratio.png', format = 'png')#, quality = 100)
plt.show()
...
plt.savefig('Plots/Pred800from795PCA_ratio.png', format = 'png')#, quality = 100)
plt.show()
...
plt.savefig('Plots/Pred800from794PCA_ratio.png', format = 'png')#, quality = 100)
plt.show()
...
plt.savefig('Plots/Pred800from793PCA_ratio.png', format = 'png')#, quality = 100)
plt.show()
...
# Write the data out...
df2.to_csv("Plots/ANNDerivWidePCA.csv", index = False )
